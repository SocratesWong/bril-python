...
function evalInstr(instr: bril.Instruction, state: State): Action {
  ...
  case "br": {
    ...
    if (cond) {
      let args = instr.args || [];
      console.log('{"op":"guard","labels":["fail"],"args":["'+ args[0]+'"]}')
      return {"action": "jump", "label": getLabel(instr, 0)};
    } else {
      // ADD not
      let args = instr.args || [];
      console.log('{"op":"not","dest":"notcond","args":["'+ args[0]+'"]}')
      console.log('{"op":"guard","labels":["fail"],"args":["notcond"]}')
